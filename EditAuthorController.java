...
public class EditAuthorController implements Initializable
{
    ...
    final ObservableList<String> authors = FXCollections.observableArrayList();
    ...
    public void save() {
        try{
            ...
            authorAdapter.editAuthorInfo(nameField.getText(), contractField.getText(),emailField.getText(),addressArea.getText(),publisherBox.getValue()+"");
    ...
    public void delete()
    {
        try
        {
            authorAdapter.deleteAuthor(nameField.getText());
        }
        catch(SQLException ex)
        {
            displayAlert("ERROR: " + ex.getMessage());
    ...
    public void buildComboBoxData() {
        ...
        try
        {
            authors.addAll(authorAdapter.getAuthorNames());
        }
        catch (SQLException ex) {
            displayAlert("ERROR: " + ex.getMessage());
        }
    ...
    public void autoFill() throws SQLException
    {
        try
        {
            String name = authorBox.getValue()+"";
            info = authorAdapter.getAuthorInfo(name);
            nameField.setText(info.get(0));
            contractField.setText(info.get(1));
            emailField.setText(info.get(2));
            addressArea.setText(info.get(3));
            publisherBox.setValue(info.get(4));
        }
        catch(SQLException ex)
        {
            displayAlert("ERROR: "+ex.getMessage());
